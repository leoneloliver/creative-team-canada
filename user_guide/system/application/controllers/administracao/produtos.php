...
class Produtos extends Controller {
	...
	function cadastrar(){
		...
		if($this->form_validation->run() == FALSE){
			...
			if(!$this->upload->do_upload()){
				...
				// -----  para pegar uma imagem especifica com nome do campo//
				$field_name = "userfile2";
				$this->upload->do_upload($field_name);
				$arquivo_enviado2 = $this->upload->data();
				// -----//
				$data['foto2'] = $arquivo_enviado2['file_name'];
	...
	function gravar_alteracao(){		
		...
		if($_FILES['userfile2']['size'] > 0){
			...
			$this->load->library('upload', $config);
			if($this->upload->do_upload()){
				$foto_antiga2 = $this->dados_produto($this->input->post('id'));
				unlink('./imgs/'.$foto_antiga2[0]->foto);
				$arquivo_enviado2 = $this->upload->data();
				$data['foto2'] = $arquivo_enviado2['file_name'];
			}
		}
